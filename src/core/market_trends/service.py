...
class MarketTrendService:
    ...
    def trend_for_asset(
        ...
        cache_key = (asset_id, current_value, league_id, boundary, generation, TREND_METHOD_VERSION)
        with self._lock:
        ...
        source = evidence or self.reader.market_trend_evidence(
            asset_ids=(asset_id,), league_id=league_id, as_of=boundary,
        ).get(asset_id, {})
    ...
    def summaries(
        ...
        as_of: str, generation: str,
    ) -> dict[str, dict[str, Any]]:
        ids = tuple(str(row["asset_id"]) for row in assets[:250])
        evidence = self.reader.market_trend_evidence(asset_ids=ids, league_id=league_id, as_of=as_of)
