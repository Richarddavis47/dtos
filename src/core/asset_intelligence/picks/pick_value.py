...
def dynasty_pick_value(pick: dict[str, Any]) -> AssetEvaluation:
    ...
    slot = str(pick.get("projected_range") or pick.get("projected_slot") or pick.get("slot") or "").lower()
    slot_adjustment = (
        10 if slot == "early"
        else -10 if slot == "late"
        else 0
    )
    ...
    # Unknown is an uncertainty-aware neutral estimate, not a premium outcome.
    if not slot:
        slot_adjustment = -4
