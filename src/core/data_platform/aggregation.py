...
def consensus(key: str, rows: tuple[DataEnvelope, ...], expected: tuple[str, ...]) -> ConsensusResult:
    ...
    weights = tuple(max(0.05, row.confidence / 100) / (1 + abs(float(row.value) - center) / scale) for row in available)
    ...
    confidence = round(sum(row.confidence for row in available) / len(available) * 0.45 + agreement * 0.35 + coverage * 100 * 0.2)
