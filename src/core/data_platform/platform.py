...
class DataPlatform:
    ...
    def fetch(self, provider_name: str, key: str, context: dict[str, Any], *, mode: str = "online", allow_cached: bool = True) -> DataEnvelope:
        ...
        if not meta.enabled:
            return self._unavailable(key, meta.category, provider_name, now, "Provider disabled by licensing or deployment configuration.")
        if mode == "offline":
            cached = self._cache.get(cache_key) if allow_cached else None
            if cached:
                return replace(cached, freshness="stale", cache_state="historical_snapshot", retrieval_mode="cached_fallback", confidence=max(0, cached.confidence - 20), limitations=(*cached.limitations, "Live provider unavailable; using disclosed cached snapshot."))
            historical = self.warehouse.latest(provider_name, key) if allow_cached else None
            if historical:
                return replace(historical, freshness="stale", cache_state="historical_snapshot", retrieval_mode="historical_snapshot", confidence=max(0, historical.confidence - 30), limitations=(*historical.limitations, "Live and fresh cache unavailable; using historical snapshot."))
        ...
        if allow_cached and not context.get("force_refresh") and cached and age is not None and age <= self.cache_ttl_seconds:
            return replace(cached, cache_state="fresh_cache", retrieval_mode="cache_hit", freshness="fresh")
        try:
            ...
            envelope = replace(envelope, cache_state="fresh", retrieval_mode="live")
            ...
            cached = self._cache.get(cache_key) if allow_cached else None
            if cached:
                return replace(cached, freshness="stale", cache_state="fresh_cache", retrieval_mode="cached_fallback", confidence=max(0, cached.confidence - 20), limitations=(*cached.limitations, f"Live provider failed: {exc}"))
            historical = self.warehouse.latest(provider_name, key) if allow_cached else None
            if historical:
                return replace(historical, freshness="stale", cache_state="historical_snapshot", retrieval_mode="historical_snapshot", confidence=max(0, historical.confidence - 30), limitations=(*historical.limitations, f"Live provider and fresh cache failed: {exc}"))
            if context.get("dtos_estimate") is not None:
                return self._estimate(key, meta.category, provider_name, now, context["dtos_estimate"])
    ...
    def health(self) -> dict[str, Any]:
        return {"status": "healthy" if all(row.status not in {ProviderStatus.DEGRADED, ProviderStatus.RATE_LIMITED} for row in self._health.values()) else "degraded", "providers": {name: asdict(row) for name, row in self._health.items()}, "cache": {"entries": len(self._cache), "ttl_seconds": self.cache_ttl_seconds}, "snapshots": sum(len(self.warehouse.history(key[2])) for key in self._cache)}
    ...
    def _estimate(key: str, category: str, provider: str, timestamp: str, value: Any) -> DataEnvelope:
        return DataEnvelope(key, category, value, "DTOS deterministic estimate", provider, timestamp, "estimated", 35, "none", "dtos_estimate", DataQuality("warning", ("External provider data unavailable",), 50), ("DTOS estimate is not provider consensus.",))

    @staticmethod
    def _unavailable(key: str, category: str, provider: str, timestamp: str, reason: str) -> DataEnvelope:
        return DataEnvelope(key, category, None, provider, provider, timestamp, "unavailable", 0, "empty", "unavailable", DataQuality("blocked", (reason,), 0), (reason, "Fallback chain exhausted: live, fresh cache, historical snapshot, and DTOS estimate unavailable."))
