...
class CanonicalTeamAssessmentTests(TestCase):
    ...
    def test_rendered_team_hq_uses_canonical_bounds_and_explicit_diagnostics(self):
        ...
        self.assertNotIn("Legacy diagnostic:", response.text)
        self.assertIn("Actual starters · projected points", response.text)
        self.assertIn("Optimal legal lineup · projected points", response.text)
        self.assertIn("Evidence generation:", response.text)
