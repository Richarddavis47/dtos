...
.ti-builder{display:grid;grid-template-columns:1fr 1fr;gap:12px}.ti-builder label{display:grid;gap:5px;color:var(--muted);font-size:12px}.ti-builder select{width:100%;background:#07111f;color:var(--text);border:1px solid var(--line);border-radius:8px;padding:9px}.ti-result{margin-top:12px;padding:12px;border:1px solid var(--line);border-radius:10px;background:#07111f;white-space:pre-wrap}.ti-result[hidden]{display:none}
...
def trade_workflow(view: dict, workflow: str) -> str:
    labels = {
    ...
    return f"""{TRADE_CSS}<section class="card ti-hero"><div><div class="identity-kicker">Trade Center · {escape(title)}</div><h2>{escape(title)}</h2><p>{escape(description)}</p></div><a class="ti-action" href="/trades?front_office={roster_id}">All Trade Workflows</a></section><section class="card" id="trade-builder" data-trade-workflow="{escape(workflow)}" data-front-office="{roster_id}" data-workspace-endpoint="/api/trades/workspace" data-evaluate-endpoint="/api/trades/evaluate" data-generate-endpoint="/api/trades/generate"><h3>Start here</h3><p>{escape(intent)}</p><div class="ti-builder"><label>Counterparty<select id="trade-partner" aria-label="Counterparty"></select></label><label>Asset you send<select id="trade-sent" aria-label="Asset you send"></select></label><label>Asset you receive or target<select id="trade-received" aria-label="Asset you receive or target"></select></label></div><div class="ti-actions"><button class="ti-action" id="trade-run" type="button">{escape('Evaluate Trade' if workflow == 'create' else 'Find Bilateral Options')}</button><button class="ti-action" id="trade-edit" type="button">Edit Trade</button><button class="ti-action" id="trade-adjust" type="button">Adjust Offer</button></div><div class="ti-result" id="trade-result" role="status" aria-live="polite" hidden></div><p class="muted">Ownership is revalidated before any generated proposal is presented as executable. DTOS analyzes trades but does not submit offers.</p></section><script>
(()=>{{const root=document.getElementById('trade-builder'),partner=document.getElementById('trade-partner'),sent=document.getElementById('trade-sent'),received=document.getElementById('trade-received'),result=document.getElementById('trade-result');let workspace=null;const active=Number(root.dataset.frontOffice);function option(select,value,label){{const row=document.createElement('option');row.value=value;row.textContent=label;select.appendChild(row)}}function assets(){{sent.replaceChildren();received.replaceChildren();const activeTeam=workspace.teams.find(team=>team.roster_id===active),other=workspace.teams.find(team=>team.roster_id===Number(partner.value));for(const asset of activeTeam?.assets||[])option(sent,asset.asset_id,asset.label);for(const asset of other?.assets||[])option(received,asset.asset_id,asset.label)}}fetch(root.dataset.workspaceEndpoint+'?front_office='+active).then(response=>response.json()).then(data=>{{workspace=data;for(const team of data.teams)if(team.roster_id!==active)option(partner,team.roster_id,team.team_name);partner.addEventListener('change',assets);assets()}}).catch(()=>{{result.hidden=false;result.textContent='Trade workspace is currently unavailable.'}});document.getElementById('trade-run').addEventListener('click',async()=>{{if(!workspace)return;const workflow=root.dataset.tradeWorkflow.replace('-','_'),manual=workflow==='create',payload={{workflow,active_roster_id:active,partner_roster_id:Number(partner.value),assets_sent:[sent.value],assets_received:[received.value],asset_id:workflow==='shop'?sent.value:received.value}};const response=await fetch(manual?root.dataset.evaluateEndpoint:root.dataset.generateEndpoint,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(payload)}}),body=await response.json();result.hidden=false;if(!response.ok){{result.textContent=body.detail||'Trade evaluation failed.';return}}const evaluation=manual?body.evaluation:body.results?.[0]?.evaluation;result.textContent=evaluation?evaluation.recommendation+' — '+evaluation.dominant_reason:(body.quiet_state||'No legitimate construction found.')}});document.getElementById('trade-edit').addEventListener('click',()=>sent.focus());document.getElementById('trade-adjust').addEventListener('click',()=>{{result.hidden=false;result.textContent='Use protected and excluded assets to constrain the next generated search.'}})}})();
</script>"""
