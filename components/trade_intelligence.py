...
def trade_card(dossier: TradeDossier, value_impact: dict | None = None) -> str:
    ...
    expected_display = f'{rec.expected_value:+d}' if rec.expected_value is not None else 'Unavailable'
    future_display = f'{impact.future_outlook:+d}' if impact.future_outlook is not None else 'Unavailable'
    asset_display = f'{impact.asset_value:+d}' if impact.asset_value is not None else 'Unavailable'
    ...
    integrated = ""
    if value_impact:
        def display_impact(key: str, precision: int = 1) -> str:
            value = value_impact.get(key)
            return f'{value:+.{precision}f}' if value is not None else 'Unavailable'
        integrated = f'<section class="ti-section"><h4>Price & Evidence Impact</h4><p><b>Intrinsic dynasty benefit:</b> {display_impact("dtos_dynasty")}</p><p><b>Acquisition-price delta:</b> {display_impact("market")}</p><p><b>Contender value:</b> {display_impact("contender")}</p><p><b>Rebuilder value:</b> {display_impact("rebuild")}</p><p><b>Weekly projection delta:</b> {display_impact("weekly", 2)}</p></section>'
    return f"""
<article class="ti-card"><div class="ti-head"><div><div class="ti-priority">{escape(rec.priority.value)} · {escape(rec.trade_type.value)} · {escape(dossier.proposal.package_type)}</div><h3>{escape(rec.title)}</h3><p class="muted">{escape(dossier.executive_summary)}</p></div><div class="pill">{dossier.partner.compatibility_score}% compatibility<br>{escape(dossier.partner.difficulty)}</div><div class="ti-score">{expected_display}<small>Expected Value</small></div></div>
...
<div class="ti-metrics"><div class="ti-metric"><b>{f"{impact.current_outlook:+d}" if impact.current_outlook is not None else "Unavailable"}</b><span>Current Outlook</span></div><div class="ti-metric"><b>{future_display}</b><span>Future Outlook</span></div><div class="ti-metric"><b>{impact.positional_depth:+d}</b><span>Depth</span></div><div class="ti-metric"><b>{asset_display}</b><span>Asset Value</span></div><div class="ti-metric"><b>{rec.confidence}%</b><span>Confidence</span></div></div>
