...
def front_office_center(view: dict) -> str:
    ...
    return f'''{CSS}{primary}<section class="card foi-hero"><div><div class="identity-kicker">Franchise Management Profile</div><h2>{escape(active.team_name)}</h2><p>{escape(active.executive_summary)}</p></div><form method="get"><label for="front_office">Change Front Office</label><select id="front_office" name="front_office" onchange="this.form.submit()">{options}</select></form></section>
