...
def _shell(title: str, body: str) -> HTMLResponse:
    ...
    return HTMLResponse(f'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(title)} · DTOS</title><style>{css}</style></head><body><main class="auth"><p class="good"><b>DTOS</b> · Your dynasty front office</p>{body}</main></body></html>', headers={"Cache-Control": "no-store"})
...
def create_accounts_router(*, service: AccountService, runtime_manager: Any) -> APIRouter:
    ...
    async def account_home() -> HTMLResponse:
        ...
        if context is None:
            return _shell("Welcome", '<section class="card"><h1>Welcome to DTOS</h1><p>Your dynasty front office. Sign in or create an account, then connect Sleeper to find your leagues and teams.</p><p><a class="btn" href="/account/sign-in">Sign in</a> &nbsp; <a href="/account/create">Create account</a></p></section>')
        ...
        return _shell("Account", f'<section class="card"><h1>{escape(context.display_name)}</h1><p>Linked Sleeper identity: <b>{escape(context.sleeper_username or "Resolved")}</b> <span class="muted">(resolved, ownership not independently verified)</span></p><p>Active league: <b>{escape(context.membership.league_name or context.membership.league_id)}</b><br>Your team: <b>{escape(context.membership.franchise_name or "Mapped franchise")}</b></p><p><a class="btn" href="/">Open DTOS</a> &nbsp; <a href="/account/leagues">Your leagues</a></p><form method="post" action="/account/logout"><input type="hidden" name="csrf_token" value="{escape(context.csrf_token)}"><button class="btn" type="submit">Log out</button></form></section>')
    ...
    async def sign_in_page(error: str = "") -> HTMLResponse:
        ...
        return _shell("Sign in", f'<section class="card"><h1>Sign in to DTOS</h1>{message}<form class="grid" method="post" action="/account/sign-in"><label>Username<input name="username" autocomplete="username" required></label><label>Password<input type="password" name="password" autocomplete="current-password" required></label><button class="btn" type="submit">Sign in</button></form><p class="muted">New to DTOS? <a href="/account/create">Create an account</a>.</p></section>')
    ...
    async def create_page(error: str = "") -> HTMLResponse:
        ...
        return _shell("Create account", f'<section class="card"><h1>Create your DTOS account</h1>{message}<form class="grid" method="post" action="/account/create"><label>Display name<input name="display_name" autocomplete="name" required></label><label>Username<input name="username" autocomplete="username" required></label><label>Password<input type="password" name="password" autocomplete="new-password" minlength="12" required></label><button class="btn" type="submit">Create account</button></form><p class="muted">Your DTOS account is separate from your Sleeper identity.</p></section>')
    ...
    async def recover_page(error: str = "") -> HTMLResponse:
        ...
        return _shell("Recover account", f'<section class="card"><h1>Recover your DTOS account</h1>{message}<form class="grid" method="post" action="/account/recover"><label>Username<input name="username" autocomplete="username" required></label><label>One-time recovery code<input type="password" name="recovery_code" autocomplete="one-time-code" required></label><label>New password<input type="password" name="new_password" autocomplete="new-password" minlength="12" required></label><button class="btn" type="submit">Reset password</button></form></section>')
    ...
    async def sleeper_page(error: str = "") -> HTMLResponse:
        ...
        message = f'<p class="error">{escape(error)}</p>' if error else ""
        return _shell("Connect Sleeper", f'<section class="card"><h1>Connect your Sleeper account</h1><p>Enter your Sleeper username so DTOS can resolve your public Sleeper identity and find your leagues.</p>{message}<form class="grid" method="post" action="/account/sleeper"><input type="hidden" name="csrf_token" value="{escape(context.csrf_token)}"><label>Sleeper username<input name="sleeper_username" required></label><button class="btn" type="submit">Find my Sleeper account</button></form><p class="muted">This links a resolved Sleeper identity. Sleeper does not provide DTOS with proof that you control the account.</p></section>')
