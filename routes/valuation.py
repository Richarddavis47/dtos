...
def create_valuation_router(*, ensure_fresh: EnsureFresh, require_data: RequireData, state: dict[str, Any], page: PageRenderer | None = None) -> APIRouter:
    ...
    async def valuation_agreement() -> Any:
        report = await intelligence()
        return _intelligence_envelope(report, {"average": report["summary"].get("average_agreement"), "strongest_consensus": report["summary"].get("strongest_consensus", []), "most_disputed": report["summary"].get("most_disputed", [])})
    ...
    if page is not None:
        ...
        async def brain_dashboard() -> HTMLResponse:
            ...
            health = {**health, "agreement": health.get("agreement") if health.get("agreement") is not None else "Unavailable"}
