    ...
    async def market_page(
        ...
        if detail:
            ...
            if forward:
                ...
                forward_html = f'''<section class="card"><p class="eyebrow">Forward Outlook</p><div class="summary-grid"><article class="metric"><b>{weekly_display}</b><span>Next Week</span></article><article class="metric"><b>{forward.get("weekly_floor") if forward.get("weekly_floor") is not None else "Unavailable"}–{forward.get("weekly_ceiling") if forward.get("weekly_ceiling") is not None else "Unavailable"}</b><span>Floor–Ceiling</span></article><article class="metric"><b>{forward.get("rest_of_season_points") if forward.get("rest_of_season_points") is not None else "Unavailable"}</b><span>Rest of Season</span></article><article class="metric"><b>{forward.get("projection_confidence", 0)}%</b><span>Projection Confidence</span></article></div><p><b>Expected role:</b> {escape(str(forward.get("expected_role") or "Unknown"))}. This is DTOS Forward Production evidence, not a Sleeper projection.</p></section>'''
