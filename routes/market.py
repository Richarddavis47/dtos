    ...
    *, require_data: RequireData, state: dict[str, Any], league_id: str,
    ...
    def market_page(
        ...
        def render_body() -> bytes:
            ...
            body = f'''<style>.market-hero{{display:grid;grid-template-columns:1.2fr .8fr;gap:20px;align-items:end;min-height:220px;padding:28px;border-bottom:1px solid var(--line);background:radial-gradient(circle at 85% 10%,rgba(71,167,255,.14),transparent 30%)}}.market-hero h2{{font-size:clamp(34px,5vw,55px);line-height:1;margin:6px 0}}.market-filters{{display:grid;grid-template-columns:2fr repeat(3,1fr) auto;gap:8px;align-items:end;padding:12px 0;border:0;background:transparent;box-shadow:none}}.market-filters h3{{grid-column:1/-1}}.market-filters label{{display:grid;gap:5px}}.market-grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}}.market-asset{{position:relative;display:grid;grid-template-columns:auto 1fr auto;gap:8px 12px;align-items:center;padding:16px;border:1px solid var(--line);border-radius:17px;background:linear-gradient(145deg,var(--surface-2),var(--surface-1))}}.market-asset[data-rank="1"]{{border-color:rgba(242,189,66,.62)}}.market-asset[data-rank="2"]{{border-color:rgba(190,202,216,.45)}}.market-asset[data-rank="3"]{{border-color:rgba(190,120,72,.5)}}.market-rank{{display:grid;place-items:center;width:34px;height:34px;border-radius:50%;background:#17293a;color:var(--gold);font-weight:950}}.market-asset[data-rank="1"] .market-rank{{background:linear-gradient(145deg,#ffe599,#d99b16);color:#071108}}.market-owner{{grid-column:2;color:var(--muted);font-size:11px}}.market-value{{grid-column:3;grid-row:1/3;text-align:right;color:var(--blue);font-size:23px;font-weight:950}}.market-value small{{display:block;color:var(--muted);font-size:9px;text-transform:uppercase}}.market-context{{grid-column:2/4;padding-top:10px;border-top:1px solid var(--line);color:var(--muted);font-size:11px}}.market-open{{grid-column:1/4;text-align:right;color:var(--accent);font-weight:850}}@media(max-width:760px){{.market-hero{{grid-template-columns:1fr;padding:20px 4px;min-height:180px}}.market-filters{{grid-template-columns:1fr 1fr}}.market-filters h3,.market-filters label:first-of-type,.market-filters button{{grid-column:1/-1}}.market-grid{{grid-template-columns:1fr}}}}</style><section class="market-hero"><div><p class="eyebrow">DTOS v{VERSION} · Dynasty Exchange</p><h2>Know the market.<br>Move with purpose.</h2><p class="muted">Recognizable dynasty assets, canonical values, and league ownership in one decision surface.</p></div><div class="ux-signal-rail"><div class="ux-signal"><span class="ux-signal-icon">◇</span><span><b>{result.get("total", result.get("count", 0))} assets</b><small>Current filtered market</small></span></div><div class="ux-signal"><span class="ux-signal-icon">#</span><span><b>Ranked canonically</b><small>Stable asset identity</small></span></div></div></section><form class="market-filters" method="get" action="/market" aria-label="Asset Market filters"><h3>Find your next dynasty asset</h3><label for="market-search">Player or pick<input id="market-search" name="q" value="{escape(q)}" placeholder="Josh Allen or 2028 1st"></label><label for="market-position">Position<select id="market-position" name="position"><option value="">All assets</option>{options(((item,item) for item in ("QB","RB","WR","TE","PICK")), position)}</select></label><label for="market-availability">Availability<select id="market-availability" name="availability"><option value="">All</option>{options((("rostered","Rostered"),("day_traders_free_agent","Free Agents"),("taxi","Taxi"),("retired","Retired"),("owned_pick","Picks")), availability)}</select></label><label for="market-sort">Sort<select id="market-sort" name="sort">{options(((item,item.title()) for item in ("market","intrinsic","contender","rebuilder","confidence","risk","liquidity")), sort)}</select></label>{front_office_input}<button class="btn" type="submit">Search market</button></form><section class="ux-section"><div class="ux-section-head"><h2>{'Search Results' if q else 'Top Dynasty Assets'}</h2><p>{'Exact compact matches' if q else 'Player identity, rank, value, and ownership at a glance'}</p></div><div class="market-grid">{''.join(market_cards) or '<div class="ds-empty"><b>No matching assets.</b>Adjust the current market filters.</div>'}</div><nav aria-label="Market pagination"><a href="/market?offset={max(0, offset-limit)}&limit={limit}&sort={escape(sort)}">Previous</a> · <a href="/market?offset={offset+limit}&limit={limit}&sort={escape(sort)}">Browse all assets →</a></nav><details class="technical-details"><summary>Full market evidence table</summary><div style="overflow-x:auto"><table><caption>Canonical dynasty asset rankings</caption><thead><tr><th>Rank</th><th>Asset</th><th>Owner</th><th>Market</th><th>Intrinsic</th><th>Contender</th><th>Rebuilder</th><th>Confidence</th><th>Agreement</th><th>Evidence</th></tr></thead><tbody>{''.join(table_rows) or '<tr><td colspan="10">No canonical assets match these filters.</td></tr>'}</tbody></table></div></details></section><section class="ux-section" id="market-movers"><div class="ux-section-head"><h2>Market Movement</h2><p>Only timestamped, meaningful comparison data receives movement treatment</p></div>{movers_html}</section>{expanded}'''
            body += (
                '<p class="muted market-evidence-contract">'
                "Values remain separate; unavailable evidence is never substituted."
                "</p><details class=\"technical-details\"><summary>Market provenance</summary>"
                f"<p>Dataset <code>{escape(market.dataset_version[:12])}</code> · "
                "stable tie-break: canonical asset ID</p></details>"
            )
