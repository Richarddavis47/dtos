    ...
    *, require_data: RequireData, state: dict[str, Any], league_id: str,
    ...
    def market_page(
        ...
        def render_body() -> bytes:
            ...
            previous_query = _market_query(q=q, position=position, availability=availability, sort=sort, direction=direction, front_office=front_office, offset=max(0, offset-limit), limit=limit)
            next_query = _market_query(q=q, position=position, availability=availability, sort=sort, direction=direction, front_office=front_office, offset=offset+limit, limit=limit)
            body = f'''<section class="market-hero"><p>Global values. League-specific ownership and fit.</p><span><b>{result.get("total", result.get("count", 0))}</b> matching assets</span></section><form class="market-filters" method="get" action="/market" aria-label="Asset Market filters"><h3>Find your next dynasty asset</h3><label for="market-search">Player or pick<input id="market-search" name="q" value="{escape(q)}" placeholder="Josh Allen or 2028 1st"></label><label for="market-position">Position<select id="market-position" name="position"><option value="">All assets</option>{options(((item,item) for item in ("QB","RB","WR","TE","PICK")), position)}</select></label><label for="market-availability">Availability<select id="market-availability" name="availability"><option value="">All</option>{options((("rostered","Rostered"),("day_traders_free_agent","Free Agents"),("taxi","Taxi"),("retired","Retired"),("owned_pick","Picks")), availability)}</select></label><label for="market-sort">Sort<select id="market-sort" name="sort">{options(((item,item.title()) for item in ("market","intrinsic","contender","rebuilder","confidence","risk","liquidity")), sort)}</select></label>{front_office_input}<button class="btn" type="submit">Search market</button></form><section class="ux-section"><div class="ux-section-head"><h2>{'Search Results' if q else 'Market Assets'}</h2><p>{'Exact compact matches' if q else 'Player identity, rank, value, and ownership at a glance'}</p></div><div class="market-grid">{''.join(market_cards) or '<div class="ds-empty"><b>No matching assets.</b>Adjust the current market filters.</div>'}</div><nav aria-label="Market pagination"><a href="/market?{escape(previous_query)}">Previous</a> · <a href="/market?{escape(next_query)}">Browse all assets →</a></nav><details class="technical-details"><summary>Full market evidence table</summary><div style="overflow-x:auto"><table><caption>Filtered asset results · order is not a dynasty rank</caption><thead><tr><th>Result</th><th>Asset</th><th>Owner</th><th>Market</th><th>Intrinsic</th><th>Contender</th><th>Rebuilder</th><th>Confidence</th><th>Agreement</th><th>Evidence</th></tr></thead><tbody>{''.join(table_rows) or '<tr><td colspan="10">No canonical assets match these filters.</td></tr>'}</tbody></table></div></details></section><section class="ux-section" id="market-movers"><div class="ux-section-head"><h2>Market Movement</h2><p>Only timestamped, meaningful comparison data receives movement treatment</p></div>{movers_html}</section>{expanded}'''
            body += (
